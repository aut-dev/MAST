...
class AddTask {
    ...
    if (this.$modal.length) {
      App.getBootstrap().then((bootstrap) => {
        this.modal = new bootstrap.Modal(
          document.getElementById("delete-task-modal"),
        );
        this.warningModal = new bootstrap.Modal(
          document.getElementById("warning-task-modal"),
        );
  ...
  initImage() {
    $(".js-delete-image").click((e) => {
      ...
      $(e.target)
        .parent()
        .replaceWith('<input type="hidden" name="fields[icon]" value="">');
    });
  }
  ...
  createWeeks() {
    ...
    while (total > existing.length) {
      this.$form
        .find(".field-weeksToggle")
        .append(this.createWeek(".field-weeksToggle .week", "weeksToggle"));
      this.$form
        .find(".field-weeks")
        .append(this.createWeek(".field-weeks .week", "weeks"));
      existing = this.$form.find(".field-weeks .week");
      existingToggle = this.$form.find(".field-weeksToggle .week");
    }
    this.setCurrentWeekMarker();
  ...
  createWeek(selector, name) {
    ...
    $.each(week.find(".day"), (i, item) => {
      if (name == "weeks") {
        $(item)
          .attr("name", namespace + "[fields][" + $(item).data("day") + "]")
          .val(1);
      } else {
        $(item)
          .attr("name", namespace + "[fields][" + $(item).data("day") + "]")
  ...
  initSubmit() {
    this.$form.submit((e) => {
      if (
        !this.$form.hasClass("derailed-checked") &&
        this.$form.find("[name=entryId]").val()
      ) {
        e.preventDefault();
