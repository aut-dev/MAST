  ...
  <div>
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div class="d-flex align-items-center">
        <div
          class="border border-primary rounded me-2 d-none d-md-flex chart-size"
        >
          <div
            :class="
              'item border-primary border-end' +
              (chart.size >= 3 ? ' filled' : '')
            "
            @click="saveSize(3)"
          ></div>
          ...
          ></div>
          <div
            :class="'item border-primary' + (chart.size == 12 ? ' filled' : '')"
            @click="saveSize(12)"
          ></div>
        </div>
        <a href="#" class="me-2" @click.prevent="openSettings()"
          ><i class="fa-solid fa-gear"></i
        ></a>
        <a href="#" @click.prevent="store.deleteChart(this.chart.id)"
    ...
    <component
      v-if="loaded"
      :is="this.chart.settings.chartType"
      :data="data"
      :options="options"
      ref="chartInstance"
    />
...
export default {
    ...
    options() {
      ...
      if (this.chart.settings.dataTracked == "timeSpent") {
        options.plugins.tooltip = {
          callbacks: {
            label: (item) =>
              `${item.dataset.label}: ` + this.formatTime(item.formattedValue),
          },
        };
  ...
  methods: {
    ...
    loadData() {
      if (this.chart) {
        axios
          .post("/?action=plugin-analytics/charts-data", this.chart.settings, {
