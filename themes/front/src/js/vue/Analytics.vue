  ...
  <div>
    ...
    <ul class="list-group list-group-flush mb-3">
      ...
      <li
        ...
        v-for="(task, index) in metrics.tasks"
        :key="index"
      >
        <div class="row">
          <div class="col-3">
      ...
      <div class="dropdown">
        ...
        <ul class="dropdown-menu">
          <li v-for="(label, value) in dataTracked">
            <a
              @click.prevent="createChart(value)"
              :key="value"
              class="dropdown-item"
              href="#"
              >{{ label }}</a
        ...
        >
          <div class="me-2 mb-2 d-flex align-items-center">
            ...
            <input
              ...
              :value="task.color ? task.color : '#000000'"
              title="{{ t('Choose a color') }}"
              @change="(event) => store.saveColor(task.id, event.target.value)"
            />
          </div>
          <div
            class="invalid-feedback d-block"
            v-if="store.colorError[task.id]"
          >
            {{ store.colorError[task.id] }}
          </div>
        </div>
      </div>
    </div>
    <div class="row">
      <div
        ...
        :class="'mb-5 col-12 col-md-' + chart.size"
      >
        <chart :chart="chart" />
      </div>
