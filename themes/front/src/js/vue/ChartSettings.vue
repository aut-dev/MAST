      ...
      <div class="modal-content">
        <div class="modal-body">
          <slot name="title">
            <div class="mb-3">
              ...
              <input
                type="text"
                v-model="chart.chartTitle"
                class="form-control"
              />
              <div class="invalid-feedback d-block" v-if="errors.chartTitle">
                {{ errors.chartTitle }}
              </div>
            </div>
          </slot>
          <slot name="size">
            <div class="mb-3" v-if="!chart.id">
              ...
              <select v-model="chart.size" class="form-select">
                <option
                  v-for="(label, value) in store.sizes"
                  :key="value"
                  :value="value"
                >
                  {{ label }}
          ...
          <slot name="chartType">
            <div class="mb-3">
              ...
              <select v-model="chart.settings.dataTracked" class="form-select">
                <option
                  v-for="(label, value) in store.dataTracked"
                  :key="value"
                  :value="value"
                >
                  {{ label }}
          ...
          <slot name="chartType">
            <div class="mb-3">
              ...
              <select v-model="chart.settings.chartType" class="form-select">
                <option
                  v-for="(label, value) in store.chartTypes"
                  :key="value"
                  :value="value"
                >
                  {{ label }}
            ...
            <div class="mb-3">
              ...
              <div class="form-check form-switch">
                <input
                  ...
                  role="switch"
                  id="allTasks"
                  v-model="chart.settings.allTasks"
                />
              </div>
            </div>
            <div class="mb-3">
              <div v-show="!chart.settings.allTasks">
                ...
                <div class="invalid-feedback d-block" v-if="errors.tasks">
                  {{ errors.tasks }}
                </div>
              </div>
            </div>
            ...
            <div v-if="isBarOrLine" class="mb-3">
              ...
              <div class="form-check form-switch">
                <input
                  ...
                  role="switch"
                  id="cumulative"
                  v-model="chart.settings.cumulative"
                />
              </div>
            </div>
          </slot>
          ...
          <slot name="xAxis">
            <div v-if="showXaxis" class="mb-3">
              ...
              <select v-model="chart.settings.groupBy" class="form-select">
                <option
                  v-for="(label, value) in store.groupBys"
                  :key="value"
                  :value="value"
                >
                  {{ label }}
              ...
              <div class="invalid-feedback d-block" v-if="errors.groupBy">
                {{ errors.groupBy }}
              </div>
            </div>
          </slot>
          <slot name="dates">
            <div class="mb-3">
              <label>{{ t("Date") }}</label>
              <select
                v-model="chart.settings.dateRange"
                class="form-select"
                ref="dateSelect"
              >
                <option
                  v-for="(label, value) in store.dateRanges"
                  :key="value"
                  :value="value"
                >
                  {{ label }}
            ...
            <div class="mb-3" v-show="chart.settings.dateRange == 'custom'">
              ...
              <div>
                <input
                  ...
                  v-model="chart.settings.dateFrom"
                  ref="dateFrom"
                />
              </div>
              <div class="invalid-feedback d-block" v-if="errors.dateFrom">
                {{ errors.dateFrom }}
              </div>
            </div>
            <div v-show="chart.settings.dateRange == 'custom'">
              ...
              <div>
                <input
                  type="text"
                  class="form-control datepicker"
                  v-model="chart.settings.dateTo"
                  ref="dateTo"
                />
              </div>
              <div class="invalid-feedback d-block" v-if="errors.dateTo">
                {{ errors.dateTo }}
              </div>
            </div>
          </slot>
        ...
        <div class="modal-footer">
          <button
            ...
            @click="store.openSettings = false"
          >
            {{ t("Close") }}
          </button>
...
export default {
  ...
  computed: {
    showXaxis() {
      if (
        this.chart.settings.chartType == "bar" &&
        this.chart.settings.cumulative
      ) {
        return false;
      }
  ...
  mounted() {
    ...
    import(
      /* webpackChunkName: "multiple-select" */ "../components/multiple-select"
    ).then((chunk) => {
  ...
  methods: {
    updateTasks() {
      this.chart.settings.tasks = $(this.$refs.tasksSelect).multipleSelect(
        "getSelects",
      );
    },
    saveSettings() {
      ...
      if (Object.keys(this.errors).length == 0) {
        ...
        if (!this.chart.id || !isEqual(this.store.editedChart, this.chart)) {
          ...
          if (this.chart.id) {
            let chart = this.store.charts.filter(
              (c) => c.id == this.chart.id,
            )[0];
    ...
    validate() {
      ...
      if (
        !this.chart.settings.allTasks &&
        this.chart.settings.tasks.length == 0
      ) {
      ...
      if (
        this.chart.settings.chartType == "line" &&
        !this.chart.settings.groupBy
      ) {
